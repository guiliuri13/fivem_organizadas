RegisterServerEvent("last-personagem:finishedIntro")
RegisterServerEvent("last-personagem:finishedCharacter")
...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("last-personagem:characterCreate",source)
	end
end

AddEventHandler("last-personagem:finishedIntro", function()
	--print("finishedIntro")
	-- setUData spawnController = 1
	TriggerClientEvent("last-personagem:characterCreate", source)
end)

AddEventHandler("last-personagem:finishedCharacter", function(characterNome, characterSobrenome, characterAge, currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("last-personagem:normalSpawn",source,firstspawn)
	TriggerEvent("last-barbershop:init",user_id)
end